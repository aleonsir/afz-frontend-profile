<?php
if(!class_exists('AFZFP_Settings_API')){

class AFZFP_Settings_API{
...
{
    if (!empty($args['desc'])) {
        $desc = sprintf('<span class="afzfp-help-tip dashicons dashicons-editor-help" title="%s"></span>', $args['desc']);
    } else {
        $desc = '';
    }
...
public function callback_select_page($args)
{
    ...
    $html .= sprintf('</select>');
    if ('' !== $value) {
        $html .= sprintf(' <a href='.get_edit_post_link($value).' class="button"> '.__('Edit Page', 'afzfp').'</a>');
        $html .= sprintf(' <a href='.get_permalink($value).' class="button"> '.__('View Page', 'afzfp').'</a>');
    }
...
public function callback_file($args)
{
    ...
    $label = isset($args['options']['button_label']) ? $args['options']['button_label'] : __('Choose File', 'afzfp');
...
public function script()
{
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            var afzfp_page_installed = $.urlParam('afzfp_page_installed'); //afzfp_page_installed
            ...
            if (afzfp_page_installed == 1) {
                $('#afzfp_pages-tab').addClass('nav-tab-active');
            }
